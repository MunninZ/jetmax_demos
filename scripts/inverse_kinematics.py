...
def inverse_kinematic(position):
    ...
    if 30 < theta1 < 150:
    # The servo motion range is 240 deg. 150~360+0~30 = 240
        raise ValueError('Unreachable position: x:{}, y:{}, z:{}'.format(x, y, z))
    theta1 = theta1 + 360 if theta1 <= 30 else theta1
    # 0~360 to 30~390
    theta1 = theta1 - 150
...
def pulse_to_deg(pulses):
    ...
    angle1 = pulse1 * 240 / 1000
    # 0~1000 map to 0~240
    angle2 = pulse2 * -240 / 1000 + 210
    angle3 = pulse3 * 240 / 1000 - 120
    # 0~1000 map to 210~-30
    # 0~1000 map to -120~120
    return angle1, angle2, angle3
def deg_to_pulse(angles):
    angle1, angle2, angle3 = angles
    pulse1 = angle1 * 1000 / 240
    # 0~240 map to 0~1000
